...
describe('DBT-ICE', function()  
    {
     ...
     //element(by.id('pass')).sendKeys('123456');
      ...
      //element(by.css('.menu-title.text-truncate.ng-star-inserted')).click();
            ...
            browser.sleep(5000);
      element.all(by.css('.feather.feather-edit.cursor-pointer')).get(1).click();
      //element(by.id('UDEmail')).sendKeys('john12@gmail.com');

                element(by.id("UDEmail")).clear();
            ...
            browser.sleep(2000);
            element.all(by.css('.feather.feather-edit.cursor-pointer')).then(function(arr){
