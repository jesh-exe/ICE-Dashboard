...
export class EcommerceSidebarComponent implements OnInit {
  ...
  commonCodeMultipleFilter(value, event) {
    ...
    console.log("commonCodeMultipleFilter-- ", value, event);
    let tempVal = value;
  ...
  refreshFileList() {
    ...
    for (var i = 0; i < this.onFilterSelectArray.length; i++) {
      ...
      if (localCheckboxRefernce.id.startsWith("sizeRange")) {
        ...
        console.log("localCheckboxValue", localCheckboxValue);
    ...
    this.fileupload
      ...
      .subscribe((response) => {
        console.log("getFileOnMultipleFilter", response);
        console.log("onFilterReferenceArray", this.onFilterReferenceArray);
  ...
  removefilter(data) {
    ...
    this.onFilterReferenceArray[index].selected = false;

    if (
      this.onFilterReferenceArray[index].id.startsWith("extension") &&
      this.onFilterSelectArray.length == 0
    ) {
      this.showDropdown = false;
      this.showDropdownList = false;
    }
    if (
      this.onFilterReferenceArray[index].id.startsWith("metadata") &&
      this.onFilterSelectArray.length == 0
    ) {
      this.showDropdown = false;
      this.showDropdownList = false;
    ...
    if (
      this.onFilterReferenceArray[index].id.startsWith("sizeRange") &&
      this.onFilterSelectArray.length == 0
    ) {
      this.showDropdown = false;
      this.showDropdownList = false;
